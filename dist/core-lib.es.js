...
function An(e, t = {}) {
	let n = V({}, t), r = !!n.jit, i = !!n.minify, a = n.optimize == null ? !0 : n.optimize, o = fn(n).parse(e);
...
function sr(e, t) {
	if (!__INTLIFY_DROP_MESSAGE_COMPILER__ && G(e)) {
		let n = K(t.warnHtmlMessage) ? t.warnHtmlMessage : !0;
...
function sa(e = {}) {
	let { __root: t, __injectWithOption: n } = e, i = t === void 0, a = e.flatJson, o = lt ? T : k, s = K(e.inheritLocale) ? e.inheritLocale : !0, c = o(t && s ? t.locale.value : G(e.locale) ? e.locale : Lr), l = o(t && s ? t.fallbackLocale.value : G(e.fallbackLocale) || U(e.fallbackLocale) || J(e.fallbackLocale) || e.fallbackLocale === !1 ? e.fallbackLocale : c.value), u = o(Qi(c.value, e)), d = o(J(e.datetimeFormats) ? e.datetimeFormats : { [c.value]: {} }), f = o(J(e.numberFormats) ? e.numberFormats : { [c.value]: {} }), p = t ? t.missingWarn : K(e.missingWarn) || vt(e.missingWarn) ? e.missingWarn : !0, m = t ? t.fallbackWarn : K(e.fallbackWarn) || vt(e.fallbackWarn) ? e.fallbackWarn : !0, h = t ? t.fallbackRoot : K(e.fallbackRoot) ? e.fallbackRoot : !0, g = !!e.fallbackFormat, _ = W(e.missing) ? e.missing : null, v = W(e.missing) ? oa(e.missing) : null, y = W(e.postTranslation) ? e.postTranslation : null, b = t ? t.warnHtmlMessage : K(e.warnHtmlMessage) ? e.warnHtmlMessage : !0, x = !!e.escapeParameter, S = t ? t.modifiers : J(e.modifiers) ? e.modifiers : {}, C = e.pluralRules || t && t.pluralRules, w;
	w = (() => {
		i && Yr(null);
		let t = {
			version: Fi,
...
function ya() {
	...
	return {
		...
		formatDate: (e, t, n) => n ? (0, rt.default)(e, n).format(t) : (0, rt.default)(e).format(t)
	};
}
//#endregion
...
function od() {
	function e(e, t) {
		return t ? (0, rt.default)(e, t).format(ad) : (0, rt.default)(e).format(ad);
	}
	function t(e, t) {
		return e == null ? "" : t ? (0, rt.default)(e, t).format(ad) : (0, rt.default)(e).format(ad);
	}
	return {
		toQDateFormat: e,
		formatQDate: t
...
function cd(e, t, n, r) {
	let { required: i, email: a, validateFromApi: o, ...s } = sd(r), c = t.required ? [i] : [];
	return n.forEach((e) => {
		if (e === "email") c.push(a);
...
function fd() {
	if (ud === null) throw Error("[LinID CoreLib] UI Design is not initialized. Call setUiDesign() first.");
	return ud;
}
//#endregion
...
var pd = [
	...
	"color",
	"thickness"
], Id = /* @__PURE__ */ "split.disableMainBtn.disableDropdown.persistent.noEscDismiss.noRouteDismiss.autoClose.noRefocus.noFocus.icon.iconRight.noCaps.noWrap.align.stack.stretch.dropdownIcon.cover.menuAnchor.menuSelf.menuOffset.size.outline.flat.unelevated.rounded.push.square.glossy.fab.fabMini.padding.color.textColor.dense.ripple.noIconAnimation.contentStyle.contentClass.transitionShow.transitionHide.transitionDuration".split("."), Ld = ["view", "container"], Rd = [
	"separator",
	"padding",
	"tag",
	"bordered",
	...
	"contentStyle",
	"contentActiveStyle"
], Xd = /* @__PURE__ */ "tag.sort.delay.delayOnTouchOnly.touchStartThreshold.disabled.animation.easing.handle.filter.draggable.ghostClass.chosenClass.dragClass.swapThreshold.invertSwap.invertedSwapThreshold.direction.forceFallback.fallbackClass.fallbackOnBody.fallbackTolerance.emptyInsertThreshold.scroll.scrollSensitivity.scrollSpeed".split("."), Zd = {
	"q-avatar": yd,
	"q-badge": bd,
	"q-banner": Ud,
	"q-btn": pd,
...
function tf(e, t, n) {
	let r = ef(e, `${t}.${n}`);
	if (r === void 0 && (r = ef(e, `default.${n}`)), typeof r == "object") throw Error(`[UiDesign] Value for '${t}.${n}' is a nested object or null, expected a primitive.`);
	return r;
}
function nf(e) {
	if (!(e in $d)) throw Error(`[UiDesign] The component '${e}' is not supported for UI design retrieval.`);
	return $d[e];
}
function rf() {
	let e = r(() => fd());
	function t(t, n, r) {
		let i = nf(n), a = {};
		for (let o of i) {
			let i = r?.[o] ?? tf(e.value, t, `${n}.${String(o)}`);
			i !== void 0 && (a[o] = i);
		}
		return a;
	}
...
var uf = () => df(Ze()), df = Je("LinidConfigurationStore", {
	state: () => ({
		entities: [],
		apiEndpoints: [],
		loading: !1,
	...
	actions: { async fetchConfiguration() {
		...
		try {
			let [e, t] = await Promise.all([sf(), lf()]);
	...
	static fromString(t, n, r) {
		let i = typeof r != "string" || r === "" ? [] : r.split(mf).filter((e) => e.includes(hf)).map((e) => {
			let t = e.indexOf(hf), n = e.slice(0, t), r = e.slice(t + 1);
			return Gu.fromString(n, r);
		});
		return new e(t, n, i);
	}
	toString() {
		return this.filters.map((e) => `${e.name}${hf}${e.toString()}`).join(mf);
	}
...
function bf(e) {
	return Object.assign(new yf(e), e.hooks);
}
//#endregion
...
async function xf(e) {
	return (await Promise.all(e.map(async (e) => {
		try {
			let t = await fetch(e);
			return t.ok ? (console.debug(`[LinID CoreLib] Loaded config for module: ${t.url}`), t.json()) : null;
		} catch {
			return console.error(`[LinID CoreLib] Config file not found: ${e}`), null;
		}
	}))).filter((e) => e !== null);
}
async function Sf(e) {
...
function wf(e, t) {
	return {
		...
		children: e.children?.map((e) => wf(e, t)) || [],
		meta: e.meta ? Tf(e.meta, t) : void 0
	};
}
function Tf(e, t) {
	if (typeof e == "string") return Xu().renderString(e, { config: t });
	if (Array.isArray(e)) return e.map((e) => Tf(e, t));
	if (typeof e == "object" && e) {
		let n = {};
		for (let r in e) n[r] = Tf(e[r], t);
		return n;
	}
	return e;
}
...
async function Df(e, t, n) {
	let r = await Sf(t);
	r && r.map((e) => wf(e, t)).forEach((e) => {
		e.name && n.hasRoute(e.name) && n.removeRoute(e.name), n.addRoute(e);
	});
	let i = xu(await Cf(t), (e) => Xu().renderString(e, { config: t }));
	if (i) {
...
async function Af(e, t, n) {
	let r = Qe();
	return t.zones?.forEach((e) => {
		"component" in e && e.component ? r.registerComponent(e.zone, e.component, e.props) : "plugin" in e && e.plugin && r.registerPlugin(e.zone, e.plugin, e.props);
	}), e.postInit(t);
}
var jf = {
	[_f.SETUP]: Ef,
...
async function Mf(e) {
	let { router: t, remotes: n, modules: r, localComponents: i } = e, a = { ...jf };
	Object.entries(e.hooks ?? {}).forEach(([e, t]) => {
		let n = jf[e];
		a[e] = async (e, r, i) => (await n(e, r, i), t(e, r, i));
	}), ee(n), ue(F()), i && he(i);
	let o = await xf(r), s = /* @__PURE__ */ new Map();
	for (let e of o) {
		let t = await de().loadRemote(e.lifecycleRemote);
		s.set(e.instanceId, t?.default);
	}
	let c = [
		_f.SETUP,
		_f.CONFIGURE,
	...
	for (let e of c) for (let n of o) await a[e](s.get(n.instanceId), n, t);
}
var Nf = { init: Mf };
//#endregion
export { vf as BasicRemoteModule, Ca as DEFAULT_DATE_FORMAT, Hu as LINID_FILTER_NEGATION_PREFIX, Vu as LINID_FILTER_OR_SEPARATOR, Gu as LinidFilter, gf as LinidFilterSet, Wu as LinidFilterValue, et as LinidZoneRenderer, _f as ModuleLifecyclePhase, ad as QDATE_DEFAULT_MASK, Lu as changeLocale, bf as createModulePageLifecycle, Su as deepEqual, Cu as deepEqualUnordered, mu as deleteEntityById, _u as fromDot, lf as getApiEndpointsConfiguration, Sa as getDayjsInstance, fu as getEntities, sf as getEntitiesConfiguration, pu as getEntityById, cf as getEntityConfiguration, ou as getHttpClient, Eu as getI18nInstance, de as getModuleFederation, lu as getModuleHostConfiguration, vu as getNestedValue, Xu as getNunjucksEnv, Ze as getPiniaStore, fd as getUiDesign, bu as isObject, Nf as linidModuleFederation, fe as loadAsyncComponent, gu as merge, me as registerLocalComponent, he as registerLocalComponents, cu as registerModuleHostConfiguration, xu as renameKeys, ge as resolveLocalComponent, Fu as resolveLocale, uu as saveEntity, xa as setDayjsInstance, au as setHttpClient, Tu as setI18nInstance, ue as setModuleFederation, yu as setNestedValue, Yu as setNunjucksEnv, Xe as setPiniaStore, dd as setUiDesign, Iu as syncLocale, _o as uiEventSubject, du as updateEntity, ya as useCommonMapper, wa as useDayjs, vo as useDialog, Bu as useFieldValidation, uf as useLinidConfigurationStore, Ku as useLinidFilterUrl, Du as useLinidUiStore, ju as useLinidUserPreference, ku as useLinidUserPreferenceStore, ff as useLinidUserStore, Qe as useLinidZoneStore, qu as useNotify, rd as useNunjucks, id as usePagination, od as useQuasarDate, sd as useQuasarFieldValidation, cd as useQuasarRules, Mu as useScopedI18n, ld as useTree, rf as useUiDesign, of as useValueFormatter };
