...
| Parameter   | Type      | Required | Description                                                                                                                                                        |
|-------------|-----------|----------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `value`     | `unknown` | yes      | A date string to convert to ISO format. Must be either a valid ISO 8601 string or match the format pattern from `application.${formatKey}` (e.g., `'DD/MM/YYYY'`). |
| `formatKey` | `string`  | yes      | I18n translation key suffix (appended to `application.`). For example, `'dateFormat'` resolves to `application.dateFormat`.                                        |
...
- **Invalid dates** (e.g., `'2026-02-25'` or format mismatches) return `''`.
...
toDateISO('24/07/2025', 'dateFormat'); // '2025-07-24T00:00:00.000Z'
...
// Format mismatch
toDateISO('2024-07-20', 'dateFormat'); // '' — ISO format doesn't match locale format when 'dateFormat' is 'YYYY-MM-DD'
```
