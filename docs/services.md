...
1. **Singleton:** Only the first call sets the instance; subsequent calls log a warning and do not overwrite the
   instance.
...
The **Entity Service** provides a standardized CRUD abstraction for backend entity management.
Each operation automatically resolves the correct backend endpoint using the module’s `instanceId` and its registered
configuration.
...
This service provides a unified, module-aware way to manage backend CRUD operations while relying on shared
configuration and HTTP client infrastructure.
