...
`fromString` parses and `toString` converts the filter to an HTTP query parameter pair (`name=value`), compatible with
APIs powered by [`spring-query-filter`](https://github.com/Zorin95670/spring-query-filter). See
[`docs/filters.md`](./filters.md#6-linidfilter-class) for the full API.
...
| `LinidFilter<T>`                | Generic class representing a filter, converted to a `name=value` query parameter pair       |
